...
export default function NameStamp({ className = '' }: { className?: string }) {
    ...
    const color = useTransform(scrollY, [0, 500], ['#292A2D', '#57565D']);
    ...
    useEffect(() => {
		...
		if (!workEl || !archiveEl || !moreEl) return;

		const startFade = workEl.offsetTop;
		const endFade = archiveEl.getBoundingClientRect().bottom + window.scrollY;
		...
		>
			<div className={`fixed w-screen top-0 right-0 pointer-events-none 
				...
				<div className={`group
								...
								${activeSection === 'work' ? 'text-(--text-colour)' : 'text-(--dark-grey)'}`}
				>
					<div className='col-start-4 col-span-2 row-start-1 row-span-1 w-(--two-cell-width) h-(--cell-height)
						...
						<Reveal delay={2.75} className='flex justify-end'>
							<a
								className={`font-medium text-base 3xl:text-lg text-(--bg-colour)
										flex flex-row gap-2 items-center
										w-fit h-fit px-5 py-3 rounded-full 2xl:px-6 2xl:py-4
										${activeSection === 'work' || activeSection === 'more' ? 'bg-(--text-colour) hover:bg-(--light-black)' : 'bg-(--white) hover:bg-(--nice-grey) hover:text-(--dark-black)'}
					...
					<div className='col-start-4 col-span-2 row-start-2 row-span-1 w-(--two-cell-width) h-(--cell-height) relative'>				
						<Reveal delay={0.5}>
							<Image
								...
								fill
								className={`px-8 | ultrawide:px-20 object-contain object-right transition-opacity duration-500 ${activeSection === 'work' ? 'opacity-100' : 'opacity-0'}`}
								draggable={false}
							/>
							<Image
								...
								alt='jacqueline truong'
								fill
								className={`px-8 | ultrawide:px-20 object-contain object-right transition-opacity duration-500 ${activeSection === 'archive' ? 'opacity-100' : 'opacity-0'}`}
								draggable={false}
							/>
							<Image
								src='/jt-grey.svg'
					...
					<div className='col-start-5 col-span-1 row-start-3 row-span-1 w-(--cell-width) h-(--cell-height)
						...
						<Reveal delay={1.6}>
							<Cat className={`place-self-end size-full xl:w-4/5 transition-colors duration-500 ${activeSection === 'more' ? 'text-(--grey)' : ''}`}/>
						</Reveal>
					</div>
				</div>
			</div>
